...
EOF
    ...
    [[ "$ddMode" == '1' ]] && {
        # Build Windows post-boot network script (net.bat) that binds to the correct NIC by MAC address.
        # This fixes the common "Network 1 vs Network 2" mismatch after DD imaging.
        MACADDR=""
...
rem --- Pick adapter by MAC (best), fallback to first Up adapter ---
for /f "usebackq delims=" %%A in (`powershell -NoProfile -Command ^
  "$t=$env:TARGETMAC; $t=($t -replace ':','-').ToUpper(); ^
   $a=Get-NetAdapter -ErrorAction SilentlyContinue ^| Where-Object { $_.MacAddress -eq $t } ^| Select-Object -First 1; ^
   if(-not $a){ $a=Get-NetAdapter -ErrorAction SilentlyContinue ^| Where-Object { $_.Status -eq 'Up' } ^| Select-Object -First 1 }; ^
   if(-not $a){ $a=Get-NetAdapter -ErrorAction SilentlyContinue ^| Select-Object -First 1 }; ^
   if($a){ $a.Name }"` ) do set "ETH=%%A"

if "%ETH%"=="" goto :eof

rem --- Apply static IPv4 ---
...
rem --- Optional: enable RDP and set port (if present) ---
EOF

        if [[ "$setNet" == '1' ]] || ([[ "$setNet" == '0' ]] && [[ "$AutoNet" == '0' ]]); then
            : # static config already embedded above
        fi
...
EOF
        ...
        # Keep original behavior: create startup entry via a helper bat that extends partition and copies net.bat.
        # Original script generates /tmp/boot/net.bat in GBK; convert if iconv exists, else keep ASCII/UTF-8.
        if command -v iconv >/dev/null 2>&1; then
            iconv -f 'UTF-8' -t 'GBK' '/tmp/boot/net.bat' -o '/tmp/boot/net.bat.gbk' && mv -f '/tmp/boot/net.bat.gbk' '/tmp/boot/net.bat'
        fi
    }
