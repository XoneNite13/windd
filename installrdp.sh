...
function ufwAllowPorts() {
    local ports_csv="$1"
    local proto="${2:-tcp}"

    ensureUFW || { echo -e "[33mSkip: ufw tidak tersedia di sistem ini.[0m"; return 0; }
    ...
    for p in "${PORTS[@]}"; do
        ...
        echo -e "[32mUFW allow ${p}/${proto}[0m"
    done

    ufw status 2>/dev/null | grep -qi "Status: active" || ufw --force enable >/dev/null 2>&1 || true
    echo -e "
[36m# UFW status[0m"
...
function detectHostRelease() {
    # Auto-detect current OS if user didn't specify -d/-u/-c.
    # Only used to pick sensible defaults; user args still override.
    if [[ -n "$Relese" ]]; then
        return 0
    fi
...
if [[ "$ddMode" == '1' ]]; then
    dependence iconv
    # dd mode (install from image URL) uses Debian netboot as a tiny environment.
    # Keep arch auto-detected later; do NOT force amd64.
    linux_relese='debian'
    tmpDIST='bookworm'
    tmpVER=''
fi
...
# Gateway (most reliable): route get to internet
ipGate=$(ip route get 1.1.1.1 2>/dev/null | awk '/ via /{for(i=1;i<=NF;i++) if($i=="via"){print $(i+1); exit}}')
[ -z "$ipGate" ] && ipGate=$(ip route show default dev "$interface" 2>/dev/null | awk '{for(i=1;i<=NF;i++) if($i=="via"){print $(i+1); exit}}' | head -n1)
# Fallback: some systems show "default dev IFACE" without "via" (point-to-point). In that case, use 0.0.0.0.
if [[ -z "$ipGate" ]]; then
    ip -o -4 addr show dev "$interface" 2>/dev/null | grep -q "peer" && ipGate="0.0.0.0"
fi


    # DNS auto from resolv.conf if user didn't provide --ip-dns (or still default)
...
# --- Preflight: allow port in UFW before install/reboot ---
...
echo -e "
[36m# Firewall Preflight (UFW)[0m"
echo -e "Port default yang akan di-allow: [33m${defaultPorts}[0m"
read -r -p "Masukkan port yang ingin di-allow (comma-separated) atau ENTER untuk default: " inputPorts
portsToAllow="${inputPorts:-$defaultPorts}"
read -r -p "Protocol (tcp/udp) [tcp]: " inputProto
protoToAllow="${inputProto:-tcp}"

ufwAllowPorts "$portsToAllow" "$protoToAllow"
