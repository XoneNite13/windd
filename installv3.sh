...
## Modified for All Network Adapters support
...
function getInterface() {
    # Prefer interface used for outbound route (more reliable on modern VPS)
    local iface=""
...
if [ "$setNet" == "0" ]; then
    ...
    # IPv4/CIDR on primary interface (scope global)
    iAddr=$(ip -o -4 addr show dev "$interface" scope global 2>/dev/null | awk '{print $4}' | head -n1)

    ipAddr=$(echo ${iAddr} | cut -d'/' -f1)
    ipMask=$(netmask $(echo ${iAddr} | cut -d'/' -f2))

    # Gateway (best): route get
    ipGate=$(ip route get 1.1.1.1 2>/dev/null | awk '/ via /{for(i=1;i<=NF;i++) if($i=="via"){print $(i+1); exit}}')
    # fallback: default route on iface
    if [[ -z "$ipGate" ]]; then
        ipGate=$(ip route show default dev "$interface" 2>/dev/null | awk '{print $3}' | head -n1)
    fi
    # fallback for point-to-point routes (no 'via'): allow 0.0.0.0 so script continues;
    # Windows net config script will still try and you can override with --ip-gate if needed.
    if [[ -z "$ipGate" ]]; then
        ipGate="0.0.0.0"
    fi

    # DNS from resolv.conf if user didn't provide --ip-dns (or still default)
    if [[ -z "$ipDNS" || "$ipDNS" == "8.8.8.8" ]]; then
        tmpDNS=$(awk '/^nameserver/{print $2}' /etc/resolv.conf 2>/dev/null | head -n1)
        [[ -n "$tmpDNS" ]] && ipDNS="$tmpDNS"
    fi
fi
if [ -z "$interface" ]; then
    dependence ip
...
EOF
    ...
    [[ "$ddMode" == '1' ]] && {
        # MODIFIED SECTION: Build Windows net.bat that configures ALL network adapters
        MACADDR=""
        [[ -n "$interface" ]] && MACADDR="$(cat /sys/class/net/${interface}/address 2>/dev/null | tr '[:lower:]' '[:upper:]' || true)"
        
        # DNS: allow multiple servers if user passes "x.x.x.x y.y.y.y"
        DNS1="$(echo "$ipDNS" | awk '{print $1}')"
        DNS2="$(echo "$ipDNS" | awk '{print $2}')"
        [[ -z "$DNS1" ]] && DNS1="8.8.8.8"
        [[ -z "$DNS2" ]] && DNS2="1.1.1.1"
        [[ "$DNS1" == "$DNS2" ]] && DNS2=""

        cat >'/tmp/boot/net.bat' <<'EOFNETBAT'
@echo off
setlocal EnableExtensions EnableDelayedExpansion

rem ===================================================================
rem  Windows Network Auto-Configuration for ALL Adapters
rem  Modified to configure ALL network adapters automatically
rem ===================================================================
...
echo [%date% %time%] Starting network configuration for all adapters... > C:\netconfig.log

rem --- Loop through ALL network adapters ---
for /f "usebackq delims=" %%A in (`powershell -NoProfile -Command ^
  "Get-NetAdapter -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Name"`) do (
    
    set "ETH=%%A"
    echo [%date% %time%] Configuring adapter: !ETH! >> C:\netconfig.log
    
    rem --- Enable adapter if disabled ---
    powershell -NoProfile -Command "Enable-NetAdapter -Name '!ETH!' -Confirm:$false -ErrorAction SilentlyContinue" >nul 2>&1
    
    rem --- Remove existing IP configuration ---
    netsh interface ipv4 delete address "!ETH!" all >nul 2>&1
    netsh interface ipv4 delete dns "!ETH!" all >nul 2>&1
    
    rem --- Set static IPv4 configuration ---
    netsh interface ipv4 set address name="!ETH!" source=static address=%IPV4% mask=%MASK% gateway=%GATE% >nul 2>&1
    
    rem --- Set DNS servers ---
    netsh interface ipv4 set dns name="!ETH!" static %DNS1% >nul 2>&1
    if not "%DNS2%"=="" (
        netsh interface ipv4 add dns name="!ETH!" %DNS2% index=2 >nul 2>&1
    )
    
    echo [%date% %time%]   - IP Address: %IPV4% >> C:\netconfig.log
    echo [%date% %time%]   - Subnet Mask: %MASK% >> C:\netconfig.log
    echo [%date% %time%]   - Gateway: %GATE% >> C:\netconfig.log
    echo [%date% %time%]   - DNS: %DNS1% %DNS2% >> C:\netconfig.log
    echo. >> C:\netconfig.log
)
...
EOFNETBAT
        ...
        if [[ "$setRDP" == '1' ]] && [[ -n "$WinRemote" ]]; then
            cat >>'/tmp/boot/net.bat' <<EOFRDP

rem ===================================================================
rem  Enable Remote Desktop (RDP) Configuration
rem ===================================================================
echo [%date% %time%] Enabling RDP on port ${WinRemote}... >> C:\netconfig.log

reg add "HKLM\System\CurrentControlSet\Control\Terminal Server" /v fDenyTSConnections /t REG_DWORD /d 0 /f >nul 2>&1
netsh advfirewall firewall set rule group="remote desktop" new enable=Yes >nul 2>&1
reg add "HKLM\System\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp" /v PortNumber /t REG_DWORD /d ${WinRemote} /f >nul 2>&1
netsh advfirewall firewall add rule name="RDP ${WinRemote}" dir=in action=allow protocol=TCP localport=${WinRemote} >nul 2>&1
sc config TermService start= auto >nul 2>&1
sc start TermService >nul 2>&1

echo [%date% %time%] RDP enabled successfully on port ${WinRemote} >> C:\netconfig.log
EOFRDP
        fi

        # Add completion and connectivity test
        cat >>'/tmp/boot/net.bat' <<'EOFEND'

rem ===================================================================
rem  Network Configuration Completed
rem ===================================================================
echo. >> C:\netconfig.log
echo [%date% %time%] Network configuration completed successfully! >> C:\netconfig.log
echo [%date% %time%] ================================================== >> C:\netconfig.log
echo. >> C:\netconfig.log

rem --- Display adapter status ---
echo [%date% %time%] Current adapter status: >> C:\netconfig.log
powershell -NoProfile -Command "Get-NetAdapter | Format-Table Name, Status, MacAddress, LinkSpeed -AutoSize | Out-String" >> C:\netconfig.log

rem --- Test network connectivity ---
echo [%date% %time%] Testing network connectivity... >> C:\netconfig.log
ping -n 1 %GATE% >nul 2>&1
if %errorlevel%==0 (
    echo [%date% %time%] [OK] Gateway %GATE% is reachable >> C:\netconfig.log
) else (
    echo [%date% %time%] [FAIL] Gateway %GATE% is NOT reachable >> C:\netconfig.log
)

ping -n 1 8.8.8.8 >nul 2>&1
if %errorlevel%==0 (
    echo [%date% %time%] [OK] Internet connectivity working >> C:\netconfig.log
) else (
    echo [%date% %time%] [FAIL] Internet connectivity NOT working >> C:\netconfig.log
)

echo [%date% %time%] ================================================== >> C:\netconfig.log
echo Network configuration script completed. Check C:\netconfig.log for details.

exit /b 0
EOFEND

        # Convert to GBK encoding if iconv is available
        if command -v iconv >/dev/null 2>&1; then
            iconv -f 'UTF-8' -t 'GBK' '/tmp/boot/net.bat' -o '/tmp/boot/net.bat.gbk' 2>/dev/null && mv -f '/tmp/boot/net.bat.gbk' '/tmp/boot/net.bat'
        fi
    }
